...
class Admin{
    ...
    function getImages($data) {
        $query = "SELECT `g`.`GalleryID`, `g`.`TypeID`, `g`.`Title`, (SELECT GROUP_CONCAT(`d`.`DocumentFilename`) FROM  `settings_alldocuments` `d` WHERE `d`.`ReferenceID`=`g`.`GalleryID`) AS 'DocumentFilename' 
                    FROM  `gallery` `g`;";
        $res = DBController::getDataSet($query); 
        return array("return_code" => true, "return_data" => $res);
    }
    ...
    function addImages($data) {
        ...
        $p=array(array(":TypeID",$data["Type"]), array(":Title",$data["Title"]));
        $r = DBController::getDataSet($q,$p);
        if(count($r)>0){
            return array("return_code" => false, "return_data" => "Data Already Exists");
        }
            ...
            if($res){
                ...
                foreach($data["File"] as $file)
                {
                    ...
                    do
                    {
                        $temp = "g_".$data["Type"]."_".Helper::generate_string(10);
                        $newfilename = $temp.".".$ext;
                        $tnfilename = $temp."_TN.".$ext;
                    }
                    while(file_exists("../app/data/documents/".$newfilename));

                    if(file_put_contents(("../app/data/documents/".$newfilename), $filedata))
                    {
                        $tn = self::createThumbnail("../app/data/documents/".$newfilename, $_SERVER['DOCUMENT_ROOT']."/ShillongLawCollege/app/data/documents/".$tnfilename, 128);

                        $location = self::read_gps_location("../app/data/documents/".$newfilename);

                        $url = "file?type=documents&name=".$newfilename;
                        $query = "INSERT INTO  `settings_alldocuments` (`ReferenceID`, `DocumentLink`, `DocumentPath`, `DocumentFilename`, `ThumbnailPath`, `ThumbnailFilename`, `OrgiginalFilename`, `Latitude`, `Longitude`) VALUES (:ReferenceID, :DocumentLink, :DocumentPath, :DocumentFilename, :ThumbnailPath, :ThumbnailFilename, :OrgiginalFilename, :Latitude, :Longitude);";
                        $param=[
                            [":ReferenceID",$r1[0]['GalleryID']],
                            [":DocumentLink",$url],
                            [":DocumentPath","/ShillongLawCollege/app/data/documents/"],
                            [":DocumentFilename",$newfilename],
                            [":ThumbnailPath","/ShillongLawCollege/app/data/documents/"],
                            [":ThumbnailFilename",$tnfilename],
                            [":OrgiginalFilename",$file['filename']],
